...
class AuthJWT(AuthJWTConfig):
    ...
    def _create_token(
        ...
        reserved_claims = {
            "sub": subject,
            "nbf": not_before_time if not_before_time else self._get_int_from_datetime(datetime.now(timezone.utc)),
            "iat": issued_at if issued_at else self._get_int_from_datetime(datetime.now(timezone.utc)),
