...
from fastapi_auth_nanoid.nanoid import NanoID
...
class AuthJWT(AuthJWTConfig):
    ...
    def _get_jwt_identifier(self) -> str:
        """
        :return: NanoID generate random string
        """
    ...
    def _get_secret_key(self, algorithm: str, process: str) -> str:
        ...
        if algorithm in asymmetric_algorithms and not has_crypto:
            raise RuntimeError(
                "Missing dependencies for using asymmetric algorithms. run 'pip install fastapi-auth-nanoid[asymmetric]'"
            )
    ...
    def _create_token(
        ...
        not_before_time: Optional[int] = None,
        ...
        :param not_before_time: The time before which the token must not be accepted for processing
        :param issued_at: The time at which the JWT was issued
        :param jwt_id: A unique identifier for the JWT
        ...
        reserved_claims = {
            ...
            "nbf": (
                not_before_time
                if not_before_time
                else self._get_int_from_datetime(datetime.now(timezone.utc))
            ),
            "iat": (
                issued_at
                if issued_at
            ...
            "jti": jwt_id if jwt_id else self._get_jwt_identifier(),
        }
        if issuer:
    ...
    def _verifying_scopes(self, raw_token: dict) -> None:
        ...
        token_scopes = raw_token["scopes"] if raw_token.get("scopes") else []

        if len(self._required_scopes) > 0:
            for scope in self._required_scopes:
                if scope not in token_scopes:
                    raise NotEnoughPermissions(
                        status_code=401, message="Not enough permissions"
                    )
